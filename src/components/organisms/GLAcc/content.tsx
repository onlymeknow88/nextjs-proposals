import { deleteGLAcc, getGLAccAll } from "@/services/gl-acc";
...
const GLAccsContent = ({ tokens }: any) => {
  const router = useRouter();
  const [isloading, setLoading] = useState(false);
  const [glAccs, setGlAccs] = useState([]);
  ...
  const fetchAllGLAcc = useCallback(async () => {
    setLoading(true);

    const res = await getGLAccAll(limit, page, debounceValue, tokens);
  ...
  const handleDelete = useCallback(
    async (areaId: any) => {
      try {
        console.log(areaId);
        await deleteGLAcc(areaId, tokens);
        const res = await getGLAccAll(limit, page, keyword, tokens);
        const data = await res.data.result;
  ...
  useEffect(() => {
    fetchAllGLAcc();
  }, [page, limit, keyword, fetchAllGLAcc]);
