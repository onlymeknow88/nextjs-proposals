import { storeAmount, updateAmount, yearOptions } from "@/services/amount";
...
import { AmountTypes, GLAccTypes, UserTypes } from "@/services/data-types";
import { formatRupiah } from "@/helpers/util";
import { SelectComponent } from "@/components/atoms/Select";

interface AmountProps {
  tokens: string;
  amountBase64: AmountTypes;
  users: UserTypes;
}
...
const FormAmount = (props: AmountProps) => {
  const { tokens, amountBase64, users } = props;

  const amounts = JSON.parse(atob(amountBase64.toString()));
  ...
  const [amount, setAmount] = useState<AmountStateTypes>({
    gl_acc_id: amounts.gl_acc_id,
    amount: amounts.amount,
    year: amounts.year.toString(),
  });

  const [validation, setValidation] = useState<AmountStateTypes>(initialState);
  const [error, setError] = useState(false);

  const [optGlAcc, setOptGlAcc] = useState([]);
  ...
  const handleSubmit = async () => {
    const res = await updateAmount(amount, amounts.amount_id, tokens);
    switch (res.error) {
      case true:
              ...
              <div className="flex flex-col lg:w-[550px] md:flex-row gap-4 items-center">
                <Select2
                  ...
                  inputProps={{
                    classNames: {
                      inputWrapper:
                        "border-1 border-gray-400 h-[38px] text-black",
                    },
                  }}
                >
                  {(item: GLAccTypes) => (
