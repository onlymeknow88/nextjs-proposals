...
const FormArea = (props: AreaProps) => {
  const { areaBase64, users, tokens } = props;
  const areas = JSON.parse(atob(areaBase64.toString()));

  const initialState = {
    area_name: "",
  };

  const [area, setArea] = useState<AreaStateTypes>({
  ...
  const handleSubmit = async () => {
    const res = await updateArea(area, areas.area_id, tokens);
    switch (res.error) {
      case true:
