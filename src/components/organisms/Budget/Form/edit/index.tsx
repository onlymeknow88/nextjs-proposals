...
import { storeBudget, updateBudget } from "@/services/budget";
import router from "next/router";
import { getSelect2Ccow } from "@/services/ccow";
import { BudgetsTypes, CcowsTypes, UserTypes } from "@/services/data-types";
...
const FormBudget = (props: BudgetProps) => {
  const { tokens, users, budgetBase64 } = props;

  const budgets = JSON.parse(atob(budgetBase64.toString()));
  ...
  const [budget, setBudget] = useState<BudgetStateTypes>({
    budget_name: budgets.budget_name,
    ccow_id: budgets.ccow.ccow_id,
  });
  ...
  const handleSubmit = async () => {
    const res = await updateBudget(budget, budgets.budget_id,tokens);
    switch (res.error) {
      case true:
