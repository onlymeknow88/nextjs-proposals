import { deleteBudget, getBudgetAll } from "@/services/budget";
...
const BudgetContent = ({ tokens }: any) => {
  ...
  const fetchAllBudget = useCallback(async () => {
    ...
    const res = await getBudgetAll(limit, page, debounceValue, tokens);
  ...
  const handleDelete = useCallback(
    async (ID: any) => {
      try {
        console.log(ID);
        await deleteBudget(ID, tokens);
        const res = await getBudgetAll(limit, page, keyword, tokens);
        const data = await res.data.result;
  ...
  useEffect(() => {
    fetchAllBudget();
  }, [page, limit, keyword, fetchAllBudget]);
    ...
    <>
      ...
      <div className="pl-0 pr-4 lg:px-8 w-full max-w-full h-screen max-h-full">
        <div className="flex flex-col justify-center w-full lg:px-0 max-w-[90rem] mx-auto gap-3">
          <h3 className="text-xl font-semibold">All Budget</h3>
          <div className="flex lg:items-center lg:justify-between w-full flex-col md:flex-row lg:flex-row">
            <Button
