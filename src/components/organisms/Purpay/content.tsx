import { deletePurpay, getPurpayAll } from "@/services/purpay";
...
const PurpayContent = ({ tokens }: any) => {

  const router = useRouter();
  const [isloading, setLoading] = useState(false);
  const [purpays, setPurpays] = useState([]);
  ...
  const fetchAllPurpay = useCallback(async () => {
    setLoading(true);

    const res = await getPurpayAll(limit, page, debounceValue, tokens);
  ...
  const handleDelete = useCallback(
    async (areaId: any) => {
      try {
        console.log(areaId);
        await deletePurpay(areaId, tokens);
        const res = await getPurpayAll(limit, page, keyword, tokens);
        const data = await res.data.result;
  ...
  useEffect(() => {
    fetchAllPurpay();
  }, [page, limit, keyword, fetchAllPurpay]);
