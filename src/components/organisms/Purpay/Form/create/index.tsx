...
import { storePurpay } from "@/services/purpay";
...
const FormPurpay = (props: PurpayProps) => {
  ...
  const [purpay, setPurpay] = useState<PurpayStateTypes>(initialState);
  ...
  const handleSubmit = async () => {
    const res = await storePurpay(purpay, tokens);
    switch (res.error) {
      case true:
            ...
            <CardBody className="p-4 md:p-6 lg:p-8 gap-4">
              <div className="flex flex-col lg:w-[550px] md:flex-row gap-4">
                <InputComponent
                  ...
                  errorMessage={error && validation.purpay_name ? validation.purpay_name : ""}
                  label="Purpay Name"
                  size="sm"
                  variant="bordered"
